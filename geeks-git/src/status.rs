...
pub fn get_status<P>(repo_path: P, status_type: StatusType) -> GitResult<Vec<StatusItem>>
where
  P: AsRef<Path>,
{
  ...
  for entry in statuses.iter() {
    ...
    let path = match entry.head_to_index() {
      Some(diff) => diff
        ...
        .map(String::from)
        .ok_or_else(|| GitError::Generic("failed to get path to diff's new file.".to_string()))?,
      None => entry
        .path()
        .map(String::from)
        .ok_or_else(|| GitError::Generic("failed to get the path to indexed file.".to_string()))?,
    };
